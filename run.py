...
#后端分支模板所在目录
beRelease = '../web/src/main/webapp/WEB-INF/views/'

#前端上线发布分支所在目录
feRelease = '../fe-release-group/'

#前端上线发布分支地址
feReleaseGit = 'git@gitlab.youjie.com:fe-release-group/crm-pc.git'
...
def exeCmd(cmd):
    if (not isWindows) and ( ('jello' in cmd) or ('rm' in cmd)):
        cmd = 'sudo ' + cmd
...
def releaseQa():
    ...
    #拷贝静态资源到测试服务器
    cmd = 'scp -r ' + bakTmp + project + ' root@192.168.50.107:/opt/soft/tengine/html/mljr/'
    exeCmd(cmd)
